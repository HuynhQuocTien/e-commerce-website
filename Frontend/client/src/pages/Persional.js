...
import { Row, Col, Avatar, Button, Tooltip, Tabs, Form, Input, message, Empty } from 'antd';
import { UserOutlined, SettingOutlined, GoogleOutlined, PhoneOutlined, SolutionOutlined } from '@ant-design/icons';
import UserInfo from '../components/UserPage/InfoUser';
...
class Persional extends Component {
    ...
    async handleSubmitUserForm(values){
            ...
            if(values.avatar.length === 0 || values.avatar[0].url)
            {
                ...
                // userAvatarNotChange.append("file", '' );
            }
            else{
            ...
            for (let [key, value] of userAvatarNotChange.entries()) {
                console.log(`${key}: ${value}`);
              }
            
              console.log(userAvatarNotChange);
