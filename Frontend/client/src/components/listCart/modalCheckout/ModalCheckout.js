  ...
  async componentDidMount() {
    if (this.props.userId === "") {
      ...
      const data = await axiosInstance(
        `User/get-user-by-id/${this.props.userId}`
      ).then((res) => res.data);
      this.setState({
        user: { ...data },
        isMounted: true,
  ...
  handleSubmitCheckout = (values) => {
    ...
    if (this.props.userId) {
      this.props.onCreateOrder({
        ...
        note: values.note ? values.note.split("\n").join(";") : "",
      ...
      return (
        ...
        >
          <div style={{ textAlign: "center", padding: "20px" }}>
            <h2>Vui lòng đăng nhập để tiếp tục thanh toán!</h2>
    ...
    return (
      ...
      >
        <Spin spinning={this.props.isLoading} size="large">
          <Form
            onFinish={this.handleSubmitCheckout.bind(this)}
            initialValues={{
                  ...
                  <Fragment>
                    <Row>
                      <Col span={20} offset={2}>
                        <Timeline>
                          <Timeline.Item
                            dot={<ShoppingOutlined style={f_size_25} />}
                          >
                            <h4>
                              <Row>
                                <Col span={10}>Tiền mua: </Col>
                                <Col span={14}>
                                  {ParsePrice.parsePrice(this.props.total)} -
                                  VND
                                </Col>
                              </Row>
                            </h4>
                          ...
                          <Timeline.Item
                            dot={<CarOutlined style={f_size_25} />}
                          >
                            <h4>
                              <Row>
                                <Col span={10}>Vận chuyển: </Col>
                                <Col span={14}>
                                  {ParsePrice.parsePrice(this.state.feeShip)} -
                                  VND
                                </Col>
                              </Row>
                            </h4>
                          ...
                          <br></br>
                          <Timeline.Item
                            dot={<DollarCircleOutlined style={f_size_25} />}
                          >
                            <h3>
                              <Row>
                                ...
                                <Col span={14}>
                                  {ParsePrice.parsePrice(
                                    this.props.total + this.state.feeShip
                                  )}{" "}
                                  - VND
                                </Col>
                              </Row>
