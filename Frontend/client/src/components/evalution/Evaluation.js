  ...
  componentDidUpdate() {
    ...
    if (this.state.evaluationCount !== this.props.evaluations.length) {
      if (this.props.evaluations.length === 0) {
        ...
        let total = this.props.evaluations.reduce(
          (sum, ele) => (sum += ele.rating),
          0
        );
  ...
  handleSubmitEvaluationForm(e) {
    ...
    if (e.target.content.value.trim() === "") {
      ...
      if (this.props.userId === "") {
        message.warning(
          "Vui lòng đăng nhập tài khoản để đánh giá sản phẩm này!",
          3
        );
        ...
        <Skeleton loading={this.props.isLoading}>
          <div className="body-display-evaluation">
            <div style={{ textAlign: "center" }}>
              ...
              <h2 style={{ color: "red" }}>
                {this.state.averageRating ? this.state.averageRating : 5}/5
              </h2>
              <Rate
                allowHalf
                disabled
                tooltips={descriptionRating}
                value={formatDisplayStar(this.state.averageRating)}
              />
            ...
            <div className="body-active-rating-btn">
              ...
              <button
                className="active-rating-btn"
                onClick={(e) => this.handleClickEvaluationBtn(e)}
              >
          ...
          <div
            className={
              this.state.isEvaluationBtn
                ? "evaluation-container show-evaluation-container"
...
const mapDispatchToProps = (dispatch) => {
  return {
    create_evaluation: (eva) => {
      dispatch(handle_create_evaluation(eva));
    },
  };
};
